...
export const addSolanaTools = (server: McpServer) => {
    server.tool("is-init-solana-wallet", "Check if the Solana wallet is initialized", {
    }, async () => {
    ...
    server.tool("init-solana-wallet", "Initialize a new Solana wallet", {
    ...
    server.tool("deposit-solana", "Deposit tokens to your Solana wallet", {
    ...
    server.tool("show-solana-address", "Show your Solana wallet address", {
    ...
    server.tool("get-solana-balance", "Get your Solana wallet balance", {
    ...
    server.tool("issue-spl-token", "Issue a new SPL token on Solana", {
    ...
    server.tool("list-spl-token", "List all SPL tokens in address", {
    ...
    server.tool("transfer-spl-token", "Transfer SPL tokens to a specific address", {
        ...
        const result = await caller.transferSplToken({ toAddress, tokenAddress, amount })
    ...
    server.tool("buy-solana-to-address", "Buy SOL tokens to a specific address", {
        amount: z.number().describe("用户想要购买的SOL数量"),
        address: z.string().describe("用户想充值的 Solana 地址"),
    }, async ({ amount, address }) => {
        open(`https://buy.transit.finance/ramp?address=${address}&amount=${amount}`)
        return {
            content: [{
                type: "text",
